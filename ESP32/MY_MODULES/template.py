...
#from utime import sleep_ms, sleep_us, gmtime, mktime, ticks_ms, ticks_diff, localtime
...
# ouput
_ = Pin(led_gpio, Pin.OUT, value=0)
#led.off()

# input
_ = Pin(sleep_gpio, Pin.IN, Pin.PULL_UP)
...
try:
    ...
    print("cannot start ping thread", str(e))
...
############################
# BEFORE starting blynk.run thread or executing blynk.run() in main ?????
print("connect to blynk")
ret = my_blynk_private.connet_blynk(blynk_con)
...
if ret == False:
    ...
    w_sec = 90
    print("create watchdog %d sec. feed in main loop (or timer call) or else will reset" %w_sec)
...
#############################
print("make sure endless blynk.run() is executed")

try:
    import thread
    #### start blynk.run thread
    print("thread available, run Blynk.run as thread")
    _thread.start_new_thread(my_blynk_private.run_blynk, (blynk_con,))

except Exception as e:
    print("thread not available. Blynk.run() in main and process using Timer", str(e))
    from machine import Timer
    
    #####################
    # case 2: blynk.run in main
    # blynk.run() endless loop run in main
    # only case would be ESP8266, no thread module ??
    # processing called by timer
    ...
    def on_timer(x):
        print("timer call back", x)
        wdt.feed()
        process()
        # go to deep sleep or return for active sleep
        if use_deep_sleep:
            s = "deep sleep after timer callback"
            deep_sleep.disconnect_and_deep_sleep(s, timer_period_sec, wifi, blynk_con, pull_to_disable=pull_to_disable, pin_to_down= pin_to_down, pin_to_input= pin_to_input)
        else:
            pass
            
    # make sure this will feed the dog in time, ie invoque what_to_do more frequantly than wdt
    timer_period_sec = 60
        
    print("processing using timer %dsec" %timer_period_sec)
    m_timer.init(mode=Timer.PERIODIC, period=timer_period_sec*1000, callback=on_timer) # ms
    
    # sync ??
    
    print("run Blynk.run in main")
    my_blynk_private.run_blynk(blynk_con)
...
#########################
# main processing LOOP when thread available
# for timer, nothing else to do
#########################
...
while True:
    ...
    try:
        wdt.feed()
        process()
        # go to deep sleep or sleep
        if use_deep_sleep:
            s = "deep sleep after main while loop"
            deep_sleep.disconnect_and_deep_sleep(s, sec, wifi, blynk_con, pull_to_disable=pull_to_disable, pin_to_down= pin_to_down, pin_to_input= pin_to_input)
        else:
            sleep(sec)
        ...
        deep_sleep.disconnect_and_reset(s, wifi, blynk_con)
        sys.exit(1)
