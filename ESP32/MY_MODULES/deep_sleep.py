...
import shared
# stop_run = False   
# actual file in my_modules. set here, read by my_blynk_private. used to stop blynk.run thread
...
def enter_deep_sleep(ms:int, pull_to_disable=[], pin_to_down=[], pin_to_input=[]):
	...
	for p in pull_to_disable:
		...
		# Only those arguments that are specified will be set. The rest of the pin peripheral state will remain unchanged.
		# Pin.init(mode=-1, pull=-1, *, value=None, drive=0, alt=-1)
		
		p.init(mode = -1, pull=None) 
		
		# ESP8266 TypeError: 'mode' argument required  mode: Pin.IN, Pin.OUT, Pin.OPEN_DRAIN, Pin.ALT, Pin.ALT_OPEN_DRAIN, (some ports), 
	...
	for p in pin_to_input:
		...
		p.init(mode = p.IN, pull=None)
		
	print ("enter ESP deep sleep %d ms" %ms)
	deepsleep(ms)
...
def disconnect(s, wifi, blynk_con):
	...
	print("disconnect wifi and blynk:", s)
	...
	if blynk_con is not None:
		try:
			...
			# blynk log: [ERROR]: sensor done. webrepl button: 0. enter deep sleep
		except:
			pass
...
##############################
# clean DEEP SLEEP
#  disconnect, Pin and deep sleep
##############################
def disconnect_and_deepsleep(s,d, wifi=None, blynk_con=None, pull_to_disable=[], pin_to_down=[], pin_to_input=[] ):  # d in sec
    try:
        disconnect(s, wifi, blynk_con)
    except:
        pass
    ...
    enter_deep_sleep(d*1000, pull_to_disable=pull_to_disable,  pin_to_down=pin_to_down, pin_to_input=pin_to_input)
